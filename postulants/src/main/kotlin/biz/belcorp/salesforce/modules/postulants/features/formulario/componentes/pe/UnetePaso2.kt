...
class UnetePaso2(mContext: Context, view: UnetePaso2View) :
    ...
    private fun initSMSEvents() {
        txtPINSMS?.onTextChanged {
            val value = it.toString().trim()
            btn_send_sms.isEnabled = value.length == PE.MAX_VALUE_PIN
            when {
                value.length != PE.MAX_VALUE_PIN -> txtPINSMS?.setEstado(BaseInput.NEUTRO)
    ...
    override fun getLayout() = R.layout.unete_pe_paso_2
    ...
    override fun initRestriction() {
        ...
        txtCodigoConsultoraRecomienda?.setRestriction(Filters.filterNumber(PE.CODIGO_CONSULTORA_MAX_DIGITOS))
        txtCodigoConsultoraRecomienda?.inputType(InputType.TYPE_CLASS_NUMBER)

        txtPINSMS?.setRestriction(Filters.filterNumber(PE.MAX_VALUE_PIN))
    ...
    override fun initValidation() {
        txtTelefonoCelular?.addV(
            ...
            V(context, MIN_LENGTH, 9, R.string.unete_valid_celular_format)
        )
        txtTelefonoFijo?.addV(
            V(context, EMPTY, R.string.unete_valid_telefono_empty),
            V(context, MIN_LENGTH, 7, R.string.unete_valid_telefono_format)
        )
        txtDireccion?.addV(
        ...
        txtCodigoConsultoraRecomienda?.addV(
            V(context, EMPTY),
            V(
                context, MIN_LENGTH, PE.CODIGO_CONSULTORA_MAX_DIGITOS,
    ...
    override fun mostrarPlaces(lugares: List<PlaceModel>) {
        txtDireccion.mostrarResultados(lugares)
    }

    override fun buscarPlaces(cadenaBusqueda: String) {
        view.buscarPlaces("PE", cadenaBusqueda)
    }
