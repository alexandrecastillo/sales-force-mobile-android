...
import kotlinx.android.synthetic.main.unete_gt_paso_2.view.*
...
class UnetePaso2(mContext: Context, view: UnetePaso2View) :
    ...
    override fun initEvents() {
        ...
        spnCentroPoblado?.onItemSelected {
            spnCentroPoblado?.selected<ParametroUneteModel>()?.also {
                view.getLugarNivel4(it.idParametroUnete)
            }
        }

        spnZona?.onItemSelected {
            spnZona?.selected<ParametroUneteModel>()?.also {
                view.getLugarNivel5(it.idParametroUnete)
            }
        }
        ...
        txtCodigoConsultoraRecomienda?.onTextChanged {
            ...
            if (value.length == GT.CODIGO_CONSULTORA_MAX_DIGITOS)
    ...
    private fun initSMSEvents() {
        txtPINSMS?.onTextChanged {
            val value = it.toString().trim()
            btn_send_sms.isEnabled = value.length == GT.MAX_VALUE_PIN
            when {
                value.length != GT.MAX_VALUE_PIN -> txtPINSMS?.setEstado(BaseInput.NEUTRO)
    ...
    override fun getLayout() = R.layout.unete_gt_paso_2
    ...
    override fun showLugarNivel3(lst: List<ParametroUneteModel>) {
        spnCentroPoblado?.load(lst)
        val selected =
            UneteAlgorithms.getValueFromDireccion(view.getModel()?.direccion, Constant.NUMERO_CERO)
        spnCentroPoblado?.selection<ParametroUneteModel> { it?.nombre == selected }
    }
    ...
    override fun showLugarNivel4(lst: List<ParametroUneteModel>) {
        ...
        spnZona?.selection<ParametroUneteModel> { it?.nombre == selected }
    }

    override fun showLugarNivel5(lst: List<ParametroUneteModel>) {
        spnBarrioColonia?.load(lst)
        val selected =
            UneteAlgorithms.getValueFromDireccion(view.getModel()?.direccion, Constant.NUMERO_DOS)
    ...
    private fun direccionDataList(): List<String> {
        val centroPoblado = spnCentroPoblado?.selected<ParametroUneteModel>()?.nombre.orEmpty()
        val zona = spnZona?.selected<ParametroUneteModel>()?.nombre.orEmpty()
        val barrioColonia = spnBarrioColonia?.selected<ParametroUneteModel>()?.nombre.orEmpty()
        val direccion = txtDireccion?.getText().orEmpty()

        return listOf(centroPoblado, zona, barrioColonia, direccion)
    }
    ...
    override fun initRequired() {
        ...
        spnCentroPoblado?.required = true
        spnZona?.required = true
    ...
    override fun initRestriction() {
        txtTelefonoCelular?.setRestriction(Filters.filterNumber(GT.CELULAR_TELEFONO_MAX_DIGITOS))
        txtTelefonoFijo?.setRestriction(Filters.filterNumber(GT.CELULAR_TELEFONO_MAX_DIGITOS))
        txtDireccion?.setRestriction(
            arrayOf(
                Filters.filterDireccion(),
                Filters.filterMaxLength(GT.DIRECCION_MAX_LENGHT)
            )
        )
        txtCodigoConsultoraRecomienda?.setRestriction(Filters.filterNumber(GT.CODIGO_CONSULTORA_MAX_DIGITOS))
        txtCodigoConsultoraRecomienda?.inputType(InputType.TYPE_CLASS_NUMBER)
        txtPINSMS?.setRestriction(Filters.filterNumber(GT.MAX_VALUE_PIN))
    ...
    override fun initValidation() {
        ...
        spnCentroPoblado?.addValidations(
            V(context, Validation.NO_SELECTION, R.string.unete_paso2_valid_select)
        )
        spnZona?.addValidations(
            V(context, Validation.NO_SELECTION, R.string.unete_paso2_valid_select)
        )
        ...
        txtTelefonoCelular?.addV(
            V(context, EMPTY),
            V(
                context, MIN_LENGTH, GT.CELULAR_TELEFONO_MAX_DIGITOS,
        ...
        txtCodigoConsultoraRecomienda?.addV(
            V(context, EMPTY),
            V(
                context, MIN_LENGTH, GT.CODIGO_CONSULTORA_MAX_DIGITOS,
                R.string.unete_valid_codigo_consultora_format
            )
        )
