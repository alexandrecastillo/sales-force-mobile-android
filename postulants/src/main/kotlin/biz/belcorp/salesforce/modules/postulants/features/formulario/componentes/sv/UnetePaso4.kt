...
import kotlinx.android.synthetic.main.unete_sv_paso_4.view.*
...
class UnetePaso4(mContext: Context, view: UnetePaso4View) :
    ...
    override fun getLayout() = R.layout.unete_sv_paso_4
    ...
    override fun initRestriction() {
        ...
        txtDireccion?.setRestriction(
            arrayOf(
                ...
                Filters.filterMaxLength(SV.DIR_MAX_DIGITOS)
            )
        )

        txtNombreReferencia?.setRestriction(Filters.filterLetters(SV.NOMBRE_APELLIDO_REF_MAX_DIGITOS))
        txtApellidoReferencia?.setRestriction(Filters.filterLetters(SV.NOMBRE_APELLIDO_REF_MAX_DIGITOS))
        txtTelefonoCelularReferencia?.setRestriction(Filters.filterNumber(SV.CELULAR_MAX_DIGITOS))
        txtTelefonoFijoReferencia?.setRestriction(Filters.filterNumber(SV.TELEFONO_MAX_DIGITOS))

        txtNombreReferenciaNoFamiliar?.setRestriction(Filters.filterLetters(SV.NOMBRE_APELLIDO_REF_MAX_DIGITOS))
        txtApellidoReferenciaNoFamiliar?.setRestriction(Filters.filterLetters(SV.NOMBRE_APELLIDO_REF_MAX_DIGITOS))
        txtTelefonoCelularReferenciaNoFamiliar?.setRestriction(Filters.filterNumber(SV.CELULAR_MAX_DIGITOS))
        txtTelefonoFijoReferenciaNoFamiliar?.setRestriction(Filters.filterNumber(SV.TELEFONO_MAX_DIGITOS))

        txtMetaMonto?.setRestriction(Filters.filterNumber(SV.MONTO_MAX_DIGITOS))
    }
    ...
    override fun initValidation() {
        ...
        txtTelefonoCelularReferencia?.addV(
            ...
            V(
                context, Validation.MIN_LENGTH, SV.CELULAR_MAX_DIGITOS,
        ...
        txtTelefonoFijoReferencia?.addV(
            V(context, Validation.EMPTY),
            V(
                context, Validation.MIN_LENGTH, SV.TELEFONO_MAX_DIGITOS,
