...
class ProductCampaignAdapter :
    RecyclerView.Adapter<ProductCampaignAdapter.MultiMarkCampaignViewHolder>() {

    var items: List<EsikaLbel.Historical> = emptyList()


    fun actualizar(items: List<EsikaLbel.Historical>) {
        this.items = items
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MultiMarkCampaignViewHolder {
        val view = parent.inflate(R.layout.view_campaign_product)
    ...
    inner class MultiMarkCampaignViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        fun bind(item: EsikaLbel.Historical) = with(view) {
