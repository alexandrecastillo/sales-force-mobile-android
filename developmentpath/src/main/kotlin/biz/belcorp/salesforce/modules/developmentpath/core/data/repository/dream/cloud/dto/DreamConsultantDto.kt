package biz.belcorp.salesforce.modules.developmentpath.core.data.repository.dream.cloud.dto
...
@Serializable
data class DreamConsultantDto(
    @SerialName(COLLECTION_KEY)
    val dreamConsultantList: List<DreamConsultant> = emptyList()
) {
    @Serializable
    data class DreamConsultant(
        ...
        data class ProgressByCampaigns(
            ...
            val list: List<ConsultantSale>
        ){
            @Serializable
            data class ConsultantSale(
                @SerialName(KEY_CAMPAIGN)
                val campaign: String,
                @SerialName(KEY_GAIN_AMOUNT)
                val gainAmount: Float
            )
        }
    }
}
