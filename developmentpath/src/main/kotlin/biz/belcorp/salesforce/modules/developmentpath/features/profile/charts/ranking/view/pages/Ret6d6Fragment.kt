...
class Ret6d6Fragment : GraphicType1Fragment() {

    override fun getTitle() = getString(R.string.ranking_retention_6d6)
    ...
    private val viewStateObserver = Observer<RankingChartsViewState> { state ->
        when (state) {
            is RankingChartsViewState.Success -> setupChart(state.data.retention6d6.chartData)
        }
    }
