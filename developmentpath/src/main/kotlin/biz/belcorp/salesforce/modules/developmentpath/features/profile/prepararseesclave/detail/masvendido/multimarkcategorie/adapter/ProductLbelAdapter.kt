...
class ProductLbelAdapter :
    RecyclerView.Adapter<ProductLbelAdapter.MultiMarkCampaignViewHolder>() {
    ...
    inner class MultiMarkCampaignViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        fun bind(item: EsikaLbel, position: Int) = with(view) {
            ...
            var mssg = context.getString(R.string.product_bifasico)

            when (position) {
                1 -> {
                    mssg = context.getString(R.string.compra_nocturne_s_rum)
                }

                2 -> {
                    mssg = context.getString(R.string.compra_infini_absol)
                }

                3 -> {
                    mssg = context.getString(R.string.compra_mithyka)
                }
            }
