...
class VentaGananciaTipsUseCase(
    private val personaRepository: RddPersonaRepository,
    private val repository: VentaGananciaTipsRepository,
    ...
    fun obtenerTips(request: Request) {
        val single = doOnSingle { obtenerPersona(request.personaId, request.personaRol) }
            ...
            .flatMap {
                val tipsOrdered = it.sortedBy { tip -> tip.orden }
                Single.just(Pair(tipsOrdered, emptyList<Video>()))
            }
